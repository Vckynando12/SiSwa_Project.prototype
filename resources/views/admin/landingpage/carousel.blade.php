...
<script>
    ...
    document.getElementById('carouselForm').addEventListener('submit', function(event) {
        ...
        var url = id ? `/admin/carousel/update/${id}` : `{{ route('admin.landingpage.carousel.store') }}`;
        var method = id ? 'POST' : 'POST'; // Use POST for both create and update
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/admin/carousel/delete/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
